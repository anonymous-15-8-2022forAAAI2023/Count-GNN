...
class RGINLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, regularizer="basis", num_bases=None, act_func="relu", dropout=0.0):
        ...
        self.rgc_layer = GraphConv(
            in_feats=in_feat, out_feats=out_feat)
    ...
    def forward(self, g, x, etypes):
        g = self.rgc_layer(g, x)
        g = self.mlp(g)
        g = self.drop(g)
        return g


class GIN(GraphAdjModel):
        ...
        if self.share_arch:
            ...
            for p_rgin in self.p_net:
                ...
                if regularizer == "basis":
                    ...
                    new_weight = nn.Parameter(
                        torch.zeros((max_npel, p_rgin.rgc_layer.weight.size(1)), dtype=torch.float32, device=device,
                                    requires_grad=True))
                    with torch.no_grad():
    ...
    def forward(self, pattern, pattern_len, graph, graph_len):
        ...
        if self.add_enc and self.add_degree:
            ...
            # print('pattern_enc,pattern_output,indeg',pattern_enc.size(),pattern_output.size(),pattern.ndata["indeg"].size())
