...
class GraphSage(GraphAdjModel):
    ...
    def create_net(self, name, input_dim, **kw):
        ...
        for i in range(num_layers):
            rgcns.add_module("%s_rgc%d" % (name, i), dgl.nn.pytorch.conv.SAGEConv(
                in_feats=hidden_dim if i > 0 else input_dim, out_feats=hidden_dim, aggregator_type="gcn",
    ...
    def increase_input_size(self, config):
        ...
        super(GraphSage, self).increase_input_size(config)
                ...
                if regularizer == "basis":
                    if num_bases < old_max_ngel:
                        new_w_comp = nn.Parameter(
                            torch.zeros((new_max_ngel, num_bases), dtype=torch.float32, device=device,
                                        requires_grad=True))
                        with torch.no_grad():
            ...
            for p_rgcn in self.p_net:
                ...
                if regularizer == "basis":
                    if num_bases < old_max_npel:
                        new_w_comp = nn.Parameter(
                            torch.zeros((new_max_npel, num_bases), dtype=torch.float32, device=device,
                                        requires_grad=True))
                        with torch.no_grad():
                    ...
                    new_weight = nn.Parameter(
                        torch.zeros((max_npel, p_rgcn.weight.size(1)), dtype=torch.float32, device=device,
                                    requires_grad=True))
                    with torch.no_grad():
    ...
    def forward(self, pattern, pattern_len, graph, graph_len):
        ...
        for p_rgcn in self.p_net:
            o = p_rgcn(pattern, pattern_output)
