...
class GCN(GraphAdjModel):
    ...
    def create_net(self, name, input_dim, **kw):
        ...
        for i in range(num_layers):
            rgcns.add_module("%s_rgc%d" % (name, i), dgl.nn.pytorch.conv.GraphConv(
                in_feats=hidden_dim if i > 0 else input_dim, out_feats=hidden_dim,
    ...
    def increase_input_size(self, config):
        ...
        super(GCN, self).increase_input_size(config)
