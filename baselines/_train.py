...
from utils import anneal_fn, get_enc_len, load_data, get_linear_schedule_with_warmup,bp_compute_abmae
from cnn import CNN
from rnn import RNN
from txl import TXL
...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        #        print('ids',ids)
        #        print('pattern.size',pattern_len)
        #       print('graph.size',graph_len)
...
if __name__ == "__main__":
    ...
    elif train_config["model"] == "GCN":
        model = GCN(train_config)
    elif train_config["model"] == "GraphSage":
        model = GraphSage(train_config)
    elif train_config["model"] == "GIN":
